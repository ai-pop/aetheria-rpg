...
- **Архитектор Дерева мира.** Отдельный рабочий агент превращает стартовый совет в большое дерево из связанных регионов, вложенных локаций и конфликтующих фракций до прохода Worldsmith.
- **Один ответ — один момент.** Narrative GM не пишет главу или целую сцену: показывает одно изменение, немедленное последствие и точную деталь, после чего управление возвращается игроку.
- **Точное восстановление генерации.** При обрыве сети workflow замораживается на текущем агенте и шаге; Retry продолжает после последних успешных tools, не создавая другой мир поверх обломков.
